...
Branch 0.29.2 contains environments that can be used for robustness research. 

1. **Stochastic environments**, since many robust algorithms require a non-deterministic MDP for the uncertainty set to be well-defined. Also, these environments allow taking multiple samples from the same state.
    - HopperResample-v4
    - Walker2dResample-v4
    - HalfCheetahResample-v4

    These are only needed if you want the actual state transitions to be stochastic (i.e. *self.do_simulation(noisy_action)* ), but not the control cost (i.e. *self.control_cost(action)* ). If the control cost is allowed to depend on the noisy action, you can simply use a noisy action wrapper in the unmodified environments (if you do not need multi-sampling).

2. **Perturbed Environments**. These environments allow for the reset function to modify parameters of the model.
    - HopperPerturbed-v4
    - Walker2dPerturbed-v4
    - HalfCheetahPerturbed-v4

E.g., the following piece of code creates a Hopper-v4 environment with a modified stiffness value of the leg joint.

```python
    import gymnasium
    env = gymnasium.make(HopperPerturbed-v4)
    env.reset(leg_joint_stiffness=0.5)
```

Note that these additions are fully compatible additions, **no changes have been made to the original environments**. They can still be used!


## Original Gymnasium documentation (not modified)
