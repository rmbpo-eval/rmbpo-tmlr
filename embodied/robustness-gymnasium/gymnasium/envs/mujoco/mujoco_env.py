...
RESAMPLE_ENVS = ["HopperResample", "Walker2dResample", "HalfCheetahResample"]
PERTURBED_ENVS = ["HopperPerturbed", "Walker2dPerturbed", "HalfCheetahPerturbed"]
...
class BaseMujocoEnv(gym.Env[np.float64, np.float32]):
    ...
    def reset_model(self, **kwargs) -> NDArray[np.float64]:
        """
    ...
    def reset(
        ...
        **kwargs,
        ...
        if self.spec and self.spec.name in [*RESAMPLE_ENVS, *PERTURBED_ENVS]:
            ob = self.reset_model(state=state, x_pos=x_pos)
        else:
            ob = self.reset_model()
