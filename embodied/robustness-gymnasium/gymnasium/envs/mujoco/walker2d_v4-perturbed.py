...
class Walker2dEnvPerturbed(MujocoEnv, utils.EzPickle):
    """
    ## Description
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.thigh_joint_damping = self.model.dof_damping[3]
        self.leg_joint_damping = self.model.dof_damping[4]
        self.foot_joint_damping = self.model.dof_damping[5]
    ...
    def step(self, action):
        ...
        action_noise = action + self.np_random.uniform(
            low=noise_low, high=noise_high, size=action.shape
        )
        self.do_simulation(action_noise, self.frame_skip)     # stochastic perturbed model
    ...
    def reset(
        ...
        thigh_joint_damping_p: float = 0.0,
        leg_joint_damping_p: float = 0.0,
        foot_joint_damping_p: float = 0.0,
        joint_frictionloss: float = 0.0,
        friction_p: float = 0.0,
        torso_mass: Optional[float] = None,
    ):
        ob = super().reset(state=state, x_pos=x_pos, seed=seed)
        ...
        model.jnt_stiffness[6] = thigh_joint_stiffness
        # perturb leg joint*
        model.jnt_stiffness[4] = leg_joint_stiffness
        model.jnt_stiffness[7] = leg_joint_stiffness
        # perturb foot joint*
        model.jnt_stiffness[5] = foot_joint_stiffness
        model.jnt_stiffness[8] = foot_joint_stiffness
        ...
        model.actuator_ctrlrange[3] = [actuator_ctrlrange[0],
                                        actuator_ctrlrange[1]]
        model.actuator_ctrllimited[4] = self.actuator_ctrllimited
        model.actuator_ctrlrange[4] = [actuator_ctrlrange[0],
                                        actuator_ctrlrange[1]]
        model.actuator_ctrllimited[5] = self.actuator_ctrllimited
        model.actuator_ctrlrange[5] = [actuator_ctrlrange[0],
                                        actuator_ctrlrange[1]]
        # perturb thigh joint damping in percentage
        model.dof_damping[3] = self.thigh_joint_damping * (1 + thigh_joint_damping_p) 
        model.dof_damping[6] = self.thigh_joint_damping * (1 + thigh_joint_damping_p) 
        # perturb leg joint damping in percentage
        model.dof_damping[4] = self.leg_joint_damping * (1 + leg_joint_damping_p) 
        model.dof_damping[7] = self.leg_joint_damping * (1 + leg_joint_damping_p) 
        # perturb foot joint damping in percentage
        model.dof_damping[5] = self.foot_joint_damping * (1 + foot_joint_damping_p)
        model.dof_damping[8] = self.foot_joint_damping * (1 + foot_joint_damping_p)
